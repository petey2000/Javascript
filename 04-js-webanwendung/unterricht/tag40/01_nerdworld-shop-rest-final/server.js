...
app.get('/api/product/:code', (req, res) => {
  ...
  const product = csvdb.getProductByCode(code);
  if (!product) {
    res.status(404).send({ msg: 'File not found', success: false });
  } else {
    res.send(product);
  }
});
...
app.put('/api/product', (req, res) => {
  ...
  if (!result) {
    res.status(500).send({ msg: 'Product updating went wrong.', success: false });
  } else {
    csvdb.saveProductsToFile();
...
app.delete('/api/product', (req, res) => {
  ...
  if (!result) {
    res.status(500).send({ msg: 'Product deleting went wrong.', success: false });
  } else {
    csvdb.saveProductsToFile();
    res.send({ msg: 'Product deleted.', success: true });
  }
});

// CRU(D) - Delete HTTP-Request Delete
// BREA(D) - Delete
app.delete('/api/product/:code', (req, res) => {
  const { code } = req.params;
  const result = csvdb.deleteProductByCode(code);
